...
/**
 * annotator related state for a specific tab.
 *
...
export type State = {
  /**
   * Whether the user has activated annotator for this tab. This state
  ...
  /** Details of error that occurred while trying to activate annotator in this tab. */
  error?: Error;

  /**
   * Query indicating the annotation, group etc. to focus when annotator loads
   * in this tab.
   */
...
/**
 * The default annotator state for a new browser tab.
 */
...
/**
 * TabState stores the annotator related state for tabs in the current browser
 * session.
 */
export class TabState {
  ...
  /**
   * Current annotator related state for each tab.
   *
  ...
  /**
   * Mark a tab as having annotator loaded in it.
   */
  ...
  /**
   * Mark a tab as not having annotator loaded.
   */
  ...
  /**
   * Mark a tab as having encountered an error when trying to load annotator into it.
   */
  ...
  /**
   * Remove annotator related state about a given tab.
   */
  ...
  /**
   * Return the current annotator related state for a tab.
   */
  ...
  /**
   * Return `true` if annotator is loaded into a given tab.
   */
  ...
  /**
   * Return `true` if annotator is not loaded in a given tab.
   */
  ...
  /**
   * Return `true` if an error occurred while trying to load annotator into a given tab.
   */
  ...
  /**
   * Updates the annotator state for a tab.
   *
  ...
  setState(tabId: number, stateUpdate: Partial<State> | null) {
    ...
    const currentState = this._currentState.get(tabId);

    if (newState && currentState && isShallowEqual(newState, currentState)) {
      return;
    }
