import { executeFunction, executeScript, getExtensionId } from "./chrome-api";
 ...
 */
type Tab = globalThis.Browser.tabs.Tab & { id: number; url: string };
 ...
 * permissions.
 */
function checkTab(tab: globalThis.Browser.tabs.Tab): Tab {
  if (!tab.id || !tab.url) {
...
export class SidebarInjector {
  isClientActiveInTab: (tab: globalThis.Browser.tabs.Tab) => Promise<boolean>;
  injectIntoTab: (
    tab: globalThis.Browser.tabs.Tab,
    config: object
  ) => Promise<void>;
  removeFromTab: (tab: globalThis.Browser.tabs.Tab) => Promise<void>;
  requestExtraPermissionsForTab: (
    tab: globalThis.Browser.tabs.Tab
  ) => Promise<boolean>;

  constructor() {
    const pdfViewerBaseURL = browser.runtime.getURL("/pdfjs/web/viewer.html");
     ...
     */
    this.isClientActiveInTab = async (tab: globalThis.Browser.tabs.Tab) => {
     ...
     * otherwise it will be rejected with an error.
     */
    this.removeFromTab = (tab: globalThis.Browser.tabs.Tab) => {
      const tab_ = checkTab(tab);
      if (isPDFViewerURL(tab_.url)) {
     ...
     */
    this.requestExtraPermissionsForTab = async (
      tab: globalThis.Browser.tabs.Tab
    ) => {
      const tab_ = checkTab(tab);
      if (isVitalSourceURL(tab_.url)) {
    ...
    async function injectIntoRemoteDocument(tab: Tab, config: object) {
      ...
      if (type === CONTENT_TYPE_PDF) {
        ...
        if (
          ...
          !result.installedURL.includes(browser.runtime.getURL("/"))
        ) {
    ...
    function injectIntoPDF(tab: Tab, config: object) {
      ...
      return browser.tabs.update(tab.id, { url: getPDFViewerURL(tab.url) });
    }
    ...
    async function getVitalSourceViewerFrame(
      ...
      globalThis.Browser.webNavigation.GetAllFrameResultDetails | undefined
    > {
