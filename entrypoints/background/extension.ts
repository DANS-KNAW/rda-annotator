...
export class Extension {
  firstRun: (
    extensionInfo: globalThis.Browser.management.ExtensionInfo
  ) => Promise<void>;
  ...
  constructor() {
    ...
    this.firstRun = async (
      ...
      const tab = await browser.tabs.create({
        url: import.meta.env.serviceUrl + "welcome",
      });
    ...
    async function onBrowserActionClicked(tab: globalThis.Browser.tabs.Tab) {
      const tabId = tab.id!;
      const tabError = state.getState(tabId).error;
      if (tabError) {
        console.error(`Error loading Hypothesis in tab ${tabId}`, tabError);
    ...
    const onTabUpdated = (
      tabId: number,
      { status }: globalThis.Browser.tabs.TabChangeInfo,
     ...
     * state in {@link state}.
     */
    async function addOrRemoveClientFromTab(tab: globalThis.Browser.tabs.Tab) {
      const tabId = tab.id!;
    ...
    async function updateAnnotationCountIfEnabled(tabId: number, url: string) {
      ...
      // return `{ badge: false}`
      const { badge } = await browser.storage.sync.get({
