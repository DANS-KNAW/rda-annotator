...
/**
 * Queries the annotator service that provides statistics about the annotations
 * for a given URL.
 * 
 * @TODO How does this pass credentials if they are not set in this context? 
 *
...
export async function fetchAnnotationCount(uri: string): Promise<number> {
  const response = await fetch(
    import.meta.env.WXT_API_URL + "/badge?uri=" + encodeUriQuery(uri),
    {
      credentials: "include",
    }
  );
