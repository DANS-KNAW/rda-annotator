...
export default defineContentScript({
  ...
  async main(ctx) {
    ...
    if (isTopFrame) {
      ...
      // Check if extension is enabled - try messaging first, fallback to direct storage read
      // (Firefox MV3 has issues with @webext-core/messaging between content script and background)
      ...
      try {
        ...
        // Fallback: read storage directly (works in Firefox when messaging fails)
        try {
          const result = await browser.storage.local.get('extension-enabled')
          extensionEnabled = !!result['extension-enabled']
        }
        catch (storageError) {
          console.error('Failed to get extension state:', error, storageError)
        }
      }
      ...
      try {
        onMessage('scrollToAnnotation', async (message) => {
          if (!annotationManager || !message.data)
            return
      ...
      // Check if extension is enabled - try messaging first, fallback to direct storage read
      // (Firefox MV3 has issues with @webext-core/messaging between content script and background)
      let guestExtensionEnabled = false
      try {
        const state = await sendMessage('getExtensionState', undefined)
        guestExtensionEnabled = state.enabled
      }
      catch (error) {
        // Fallback: read storage directly (works in Firefox when messaging fails)
        try {
          const result = await browser.storage.local.get('extension-enabled')
          guestExtensionEnabled = !!result['extension-enabled']
        }
        catch (storageError) {
          console.error('[RDA Guest] Failed to get extension state:', error, storageError)
        }
      }

      if (guestExtensionEnabled) {
