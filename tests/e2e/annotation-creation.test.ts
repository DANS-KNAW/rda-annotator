...
test.describe('Annotation Creation', () => {
  test('fills annotation form and verifies vocabularies load', async ({
    ...
    if (!popupBox || popupBox.width === 0) {
      await page.screenshot({ path: `test-results/${browserName}-creation-popup-not-found.png` })
    ...
    if (!sidebarFrame) {
      await page.screenshot({ path: `test-results/${browserName}-creation-no-sidebar.png` })
    ...
    await expect(titleInput, 'Title input should be visible').toBeVisible({ timeout: 5000 })
    await titleInput.fill('E2E Test Annotation')

    // Wait for language dropdown to be available
    const languageCombobox = sidebarFrame.locator('input[id*="headlessui-combobox-input"]').first()
    await expect(languageCombobox, 'Language combobox should be visible').toBeVisible({ timeout: 5000 })

    // Take screenshot showing form loaded
    await page.screenshot({ path: `test-results/${browserName}-creation-form-loaded.png` })

    // Click the combobox button to open dropdown
    const comboboxButton = sidebarFrame.locator('button[id*="headlessui-combobox-button"]').first()
    await comboboxButton.click()
    await page.waitForTimeout(1000)

    // Check if dropdown options or a loading/no-results message appears
    // The options might take time to load from API
    const dropdownOptions = sidebarFrame.locator('[id*="headlessui-combobox-options"]').first()
    const optionsVisible = await dropdownOptions.isVisible().catch(() => false)

    if (optionsVisible) {
      await page.screenshot({ path: `test-results/${browserName}-dropdown-open.png` })

      // Check if there are actual options (not just loading/error message)
      const options = dropdownOptions.locator('[id*="headlessui-combobox-option"]')
      ...
      await page.screenshot({ path: `test-results/${browserName}-dropdown-not-visible.png` })
    }

    // Take final screenshot
    await page.screenshot({ path: `test-results/${browserName}-creation-form-filled.png` })
  ...
  test('clears pending annotation on page navigation', async ({
    ...
    let sidebarFrame = page.frames().find(f => f.url().includes('sidebar.html'))
    if (!sidebarFrame) {
      throw new Error('Sidebar iframe not found')
    }

    const createHeading = sidebarFrame.getByRole('heading', { name: 'Create Annotation' })
    await expect(createHeading, 'Should show Create form before navigation').toBeVisible({
      timeout: 5000,
    })

    await page.screenshot({ path: `test-results/${browserName}-before-navigation.png` })
    ...
    await page.screenshot({ path: `test-results/${browserName}-after-navigation.png` })
    ...
    if (sidebarFrame) {
      ...
      const createHeadingAfter = sidebarFrame.getByRole('heading', { name: 'Create Annotation' })

      // Either we're on Annotations view, or Create view doesn't have the old fragment
      const isOnAnnotationsView = await annotationsHeading.isVisible().catch(() => false)
      const isOnCreateView = await createHeadingAfter.isVisible().catch(() => false)
      ...
      if (isOnCreateView) {
        ...
        const fragmentValue = await fragmentTextarea.inputValue().catch(() => '')
        ...
        expect(isOnAnnotationsView, 'Should redirect to Annotations view after navigation').toBe(true)
      }
    }
  })

  // Debug test that actually submits to real API - skipped by default
  test.skip('submits annotation to real API (debug only)', async ({
    ...
    // Fill form
    await sidebarFrame.locator('input[name="title"]').fill('E2E Debug Test Annotation')

    // Select language
    const languageInput = sidebarFrame.locator('input[id*="headlessui-combobox-input"]').first()
    ...
    const resourceTypeInputs = sidebarFrame.locator('input[id*="headlessui-combobox-input"]')
    ...
    await page.screenshot({ path: `test-results/${browserName}-debug-form-filled.png` })
    ...
    await page.screenshot({ path: `test-results/${browserName}-debug-after-submit.png` })
  })
})
